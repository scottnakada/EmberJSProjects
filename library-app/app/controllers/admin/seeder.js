...
export default Ember.Controller.extend({
  ...
  actions: {
    ...
    deleteBooksAndAuthors() {
  	...
  	generateContact() {
    	const counter = parseInt(this.get('contactCounter'));

      	for (let i = 0; i < counter; i++) {
        	this.store.createRecord('contact').randomize().save().then(() => {
          		if (i === counter-1) {
            		this.set('contactCounter', 0);
            		this.set('contactDone', true);
          		}
        	});
      	}
    },
    ...
    deleteContact() {
      	this._destroyAll(this.get('contact'));

      	this.set('contactDelDone', true);
    },

  	generateInvitation() {
      	const counter = parseInt(this.get('invitationCounter'));

      	for (let i = 0; i < counter; i++) {
        	this.store.createRecord('invitation').randomize().save().then(() => {
          		if (i === counter-1) {
            		this.set('invitationCounter', 0);
            		this.set('invitationDone', true);
          		}
        	});
      	}
    },
    ...
    deleteInvitation() {
      	this._destroyAll(this.get('invitation'));

      	this.set('invitationDelDone', true);
    }
  },
